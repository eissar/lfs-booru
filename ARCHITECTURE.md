...
`server.ts` owns process startup, hardcoded runtime configuration, route selection, static-file serving, the initial index rebuild check, and request-specific behavior:

- Gallery rendering from `DerivedIndexStore` and templates.
...
`server.ts` then takes responsibility for source persistence and serving-state update inside the `/ingest` route. It serializes write-side Git/event/index work by appending the event with rollback protection around `stageAndCommit`, and applies the same event to the JSON indexes after Git commit succeeds.
...
Git operations are localized to `Init` and `stageAndCommit`. `Init` handles clone and setup. `stageAndCommit` stages selected paths and creates a commit in the library repository. The HTTP boundary chooses the staged paths and commit message.

### Rendering boundary

There are two rendering paths in the source tree:

- The HTTP route `/gallery` delegates to templates inside `src/template/` to construct the layout.
...
gallery request
  ...
  -> template functions render HTML via HtmlRenderer
...
- `/ingest` assumes `store.getCursor()` returns a non-null cursor before event append. The JSON store does not satisfy that assumption after construction unless an event has been applied or a cursor has been saved in memory.

## Trust, Error, and Failure Boundaries

### Request input

`ingestFile` validates that the multipart request includes an `image` file and that `tags` parses as a JSON array of strings. Other metadata fields are accepted as strings or parsed integers with fallback values.

### LFS server

The LFS client returns fetch responses directly. `ingestFile` checks `ok` for metadata registration and content upload before writing the pointer file. Image endpoints return the upstream content response without additional validation.
...
If `store.applyEvent` fails after a successful Git commit, the event and pointer can be committed while the served JSON indexes remain stale.
