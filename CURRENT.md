> Snapshot commit: HASH:31b9b005eaef304683da456e114ea242b3e4057c
...
```ts
const LFS_SERVER = 'http://mini-server:8080';
const conn = { url, auth, user: 'USER', repo: 'REPO' };
const lib = { path: './libraries/new/' };
```

## Source Structure

```text
server.ts                          HTTP server setup, route dispatch, and handlers
src/
  ...
  html.ts                          HTML template string utility
  ...
  template/
    gallery.ts                     Gallery page template function
    header_frag.ts                 Header HTML fragment
    image-card.ts                  Image card HTML fragment
...
| `/` | any | Redirects to `/gallery` |
| `/gallery` | any | Renders gallery HTML page from `store.listImages()` |
| `/f/items` | GET | Returns rendered HTML image card fragments for infinite scrolling |
| `/ingest` | POST | Ingests multipart image uploads, coordinates LFS upload and Git commit |
...
- `gallery()`: Constructs the entire gallery HTML page layout with headers and content.
- `header()`: Constructs the top navigation and toolbar fragment.
- `imageCard()`: Constructs the HTML card element for an individual image.
...
`CachingHtmlRenderer` implements `HtmlRenderer`. It escapes HTML via `@std/html/entities`, caches gallery pages under `index/artifacts/gallery-pages` by SHA-256 content hash of version, title, and card fragments.
...
GET /gallery
  -> store.listImages()
  -> render HTML cards via HtmlRenderer.renderImageCard()
  -> combine cards and render gallery page via HtmlRenderer.renderGalleryPage()

GET /f/items
  -> store.listImages() with limit
  -> return concatenated HTML cards
...
- The HTTP handler performs LFS upload and pointer-file write before checking for a non-null in-memory cursor.
- The HTTP handler reserves an ID before LFS upload and Git operations. Failed ingest attempts can consume IDs.
